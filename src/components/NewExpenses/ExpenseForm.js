...
const ExpenseForm = (props) => {
	...
	const submitHandler = (event) => {//бул функция отправка формы баардык инпуттардын значенияларын алып обьект кылып аны подьем состояние кылып newExpenses'ке жонотот
		event.preventDefault()//по умолчанный событыяны очурот антпесе страница перезагрузка боо берет
		if (title && amount && date) {//эгер ушул стейттердин ичинде бир нерсе болсо ичиндеги жогоруда айткандай код аткарылат
			const expensesData = {
				title: title,
		...
		} else { //болбосо  модалка чыгат
			alert('hgfd')
		}
	}

	return (/**бул жерде форманын версткасы ошол эле учурда событыялары */
